...
export async function listCryptoCurrencies({
	query,
}: {query: ListQueryParams}): Promise<CryptoCurrency[] | ElysiaCustomStatusResponse<any>> {
...
export async function detailCryptoMarketChart({
	...
	CryptoGraphDetail | ElysiaCustomStatusResponse<any>
> {
...
export async function detailCryptoDescriptionData({
	params,
}: {params: SlugParams}): Promise<CryptoDataDescription | ElysiaCustomStatusResponse<any>> {
	const slug = encodeURIComponent(params.slug);
